...
処理したい画像の含まれるディレクトリを絶対パスまたは相対パスで指定する。ディレクトリ内の"jpg（jpegも可）"、"png"、"tiff（tifも可）"、"jp2"、"bmp"及び"webp"の拡張子のファイルを順次処理する。

#### `--sourceimg`オプション

処理したい画像を絶対パスまたは相対パスで直接指定する。"jpg（jpegも可）"、"png"、"tiff（tifも可）"、"jp2"、"bmp"及び"webp"の拡張子のファイルを処理することが可能。
